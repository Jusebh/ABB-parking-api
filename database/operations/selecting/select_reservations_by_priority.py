    ...
    with Session(connect_to_database()) as session:
        try:
            ...
            stmt = (
                ...
                .join(Reservations.users)
                .join(Users.priority_groups)
                .where(ReservationsDates.date_of_reservation == date)
                .where(PriorityGroups.priority > int(priority))
                .order_by(desc(PriorityGroups.priority))
                .order_by(desc(Reservations.created_at))
            )
            try:
                result = session.scalars(stmt).first().reservation_id
                session.close()
                return result
            except:
                session.close()
