...
auth = identity.web.Auth(session=session, authority=app_config.AUTHORITY, client_id=app_config.CLIENT_ID, client_credential=app_config.CLIENT_SECRET)
...
def login():
    log_in = auth.log_in(scopes=app_config.SCOPE, redirect_uri=url_for("auth_response.response", _external=True), prompt="select_account")
...
def user_data():
    ...
    api_result = requests.get(app_config.ENDPOINT, headers={"Authorization": "Bearer " + token["access_token"]}, timeout=30).json()
